...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    ------------------------------------------------------ Fetch Total Deposit from Core ----------------------------------------------------------
with branches as (
SELECT *
FROM (
SELECT DISTINCT RIGHT(BRANCHSORTCODE, 4) AS branchcode4, RIGHT(BRANCHSORTCODE, 3) AS branchcode, BANKREGIONCODE,b2.branchname,
case when substr(BRANCHSORTCODE,5,1) in('1','2') then 'OLD' else 'NEW' end as branchType,
ROW_NUMBER() OVER(Partition By RIGHT(BRANCHSORTCODE, 3), case when substr(BRANCHSORTCODE,5,1) in('1','2') then 'OLD' else 'NEW' end Order By BRANCHSORTCODE) as branch_order
FROM WASADMIN.BRANCH b2
LEFT JOIN aib.aib_bankregion br on right(br.BRANCHCODE,4) = RIGHT(b2.BRANCHSORTCODE,4)
where lower(b2.BRANCHNAME) not like '%reserve%' and lower(b2.BRANCHNAME) not like '%reser%' and lower(b2.BRANCHNAME) not like '%reseve%' 
and RIGHT(BRANCHSORTCODE, 4) NOT IN ('1000','2000','1002','2002','9999')
) b where b.branch_order=1
)
, maxExchangeRate as
(
select distinct FROMCURRENCYCODE as FROMCURRENCYCODE,max(DATELASTMODIFIED) as maxDate
 
from wasadmin.EXCHANGERATESHISTORY 
where date(DATELASTMODIFIED)<=(select date(BFCURRENTFREEZETIME) from BANKFUSION.BFTB_PERSISTENTTAG)
and EXCHANGERATETYPE='MID'
group by FROMCURRENCYCODE
),
withExchangeRate as
(
select distinct er.FROMCURRENCYCODE as FROMCURRENCYCODE,er.RATE as RATE
 
from wasadmin.EXCHANGERATESHISTORY er
inner join maxExchangeRate er2 on er2.FROMCURRENCYCODE = er.FROMCURRENCYCODE and er.DATELASTMODIFIED = er2.maxDate
where TOCURRENCYCODE = 'ETB' and EXCHANGERATETYPE='MID'
 
union
 
select 'ETB' as FROMCURRENCYCODE,1.0 as RATE from BANKFUSION.BFTB_PERSISTENTTAG
)
,ShadowBalanceConventional as
(
select sum(sb.SHADOWCLEAREDBALANCE * e.RATE) as DepositBalance,br.branchcode,br.branchType
from wasadmin.account a
inner join WASADMIN.UBTB_SHADOWACCOUNTFREEZE sb on sb.ACCOUNTID = a.ACCOUNTID
inner join branches br on branchcode = right(a.branchsortcode,3) and br.branchtype = (case when substr(a.BRANCHSORTCODE,5,1) in('1','2') then 'OLD' else 'NEW' end)
inner join withExchangeRate e on a.ISOCURRENCYCODE= e.FROMCURRENCYCODE
inner join wasadmin.PRODUCTINHERITANCE p on a.PRODUCTCONTEXTCODE=p.PRODUCTCONTEXTCODE
where ((p.PRODUCT_ACC_PRODUCTID in ('Savings','SpecialSAV','notice','CurrentAccount','fixdep') 
and p.UBSUBPRODUCTID NOT IN ('01432','01438') and a.ACCOUNTID not in ('01304005574300','013041194227701'))
or ((a.ACCOUNTID ='01304005574300' or p.PRODUCT_ACC_PRODUCTID ='OverDrafts') and sb.SHADOWCLEAREDBALANCE > 0))
and (a.closed='N' or (a.closed='Y' and sb.SHADOWCLEAREDBALANCE <> 0))
group by br.branchcode,br.branchType
)
,ShadowBalanceIFB as
(
select sum(sb.SHADOWCLEAREDBALANCE * e.RATE) as DepositBalance,br.branchcode,br.branchType
...
inner join withExchangeRate e on a.ISOCURRENCYCODE= e.FROMCURRENCYCODE
inner join wasadmin.PRODUCTINHERITANCE p on a.PRODUCTCONTEXTCODE=p.PRODUCTCONTEXTCODE
where (p.PRODUCT_ACC_PRODUCTID IN ('IFBSpecialSAV', 'IFBSavings', 'IFBCurrent', 'MudarabaInvest')
OR p.UBSUBPRODUCTID IN ('01432', '01438')OR a.ACCOUNTID = '013041194227701')
and (a.closed='N' or (a.closed='Y' and sb.SHADOWCLEAREDBALANCE <> 0))
group by br.branchcode,br.branchType
),Total_balance as (
select db.branchcode,db.DepositBalance,db.branchType
from ShadowBalanceConventional db
UNION ALL
select db.branchcode,db.DepositBalance,db.branchType
from ShadowBalanceIFB db
)
select sum(DepositBalance) as Total_balance_core from Total_balance


---------------------------- table to store core data --------------------------------------
